...
d01=d0[,c("SOURCE","trt_functional","replicates","dis_rating","disrating_SE")]
...
d01$dis_rating=as.numeric(d01$dis_rating)
d01$disrating_SE=as.numeric(d01$disrating_SE)

d01=d01[-which(is.na(d01$dis_rating)),] # we exclude the rows that do not provide an effect size
...
dvth=data.frame(SOURCE=NA, trt_functional=NA, replicates=NA, dis_rating=NA, disrating_SE=NA)
dvst=data.frame(SOURCE=NA, trt_functional=NA, replicates=NA, dis_rating=NA, disrating_SE=NA)
dvc=data.frame(SOURCE=NA, trt_functional=NA, replicates=NA, dis_rating=NA, disrating_SE=NA)
...
funnel(A01, main="Funnel plot standard/control for disease rating")
...
funnel(A02, main="Funnel plot threshold/control for disease rating")
...
funnel(A12, main="Funnel plot threshold/standard for disease rating")
...
write.table(Re, "C:/Users/kboura01/OneDrive - University of Cyprus/AgStat/meta-analysis/r-codes for metanalysis - final/re-dis_rating.txt", row.names=F)
...
mtext("disease rating",3, line=-3, cex=3, oute=TRUE)

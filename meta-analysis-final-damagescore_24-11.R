...
d01=d0[,c("SOURCE","trt_functional","replicates","damagescore","damagescore_SE")]
...
d01$damagescore=as.numeric(d01$damagescore)
d01$damagescore_SE=as.numeric(d01$damagescore_SE)

d01=d01[-which(is.na(d01$damagescore)),] # we exclude the rows that do not provide an effect size
...
dvth=data.frame(SOURCE=NA, trt_functional=NA, replicates=NA, damagescore=NA, damagescore_SE=NA)
dvst=data.frame(SOURCE=NA, trt_functional=NA, replicates=NA, damagescore=NA, damagescore_SE=NA)
dvc=data.frame(SOURCE=NA, trt_functional=NA, replicates=NA, damagescore=NA, damagescore_SE=NA)
...
lnRRm01=lnRRm01[-c(1,8,15)]
vlnRRm01=vlnRRm01[-c(1,8,15)]
...
lnRRm12=lnRRm12[-c(8,15)]
vlnRRm12=vlnRRm12[-c(8,15)]
...
funnel(A01, main="Funnel plot standard/control for damage score")
...
funnel(A02, main="Funnel plot threshold/control for damage score")
...
funnel(A12, main="Funnel plot threshold/standard for damage score")
...
write.table(Re, "C:/Users/kboura01/OneDrive - University of Cyprus/AgStat/meta-analysis/r-codes for metanalysis - final/re-damagescore.txt", row.names=F)
...
mtext("damage score",3, line=-3, cex=3, oute=TRUE)

...
d01=d0[,c("SOURCE","trt_functional","replicates","insect_dens","insect_densSE")]
...
d01$insect_dens=as.numeric(d01$insect_dens)
d01$insect_densSE=as.numeric(d01$insect_densSE)

d01=d01[-which(is.na(d01$insect_dens)),] # we exclude the rows that do not provide an effect size
...
dvth=data.frame(SOURCE=NA, trt_functional=NA, replicates=NA, insect_dens=NA, insect_densSE=NA)
dvst=data.frame(SOURCE=NA, trt_functional=NA, replicates=NA, insect_dens=NA, insect_densSE=NA)
dvc=data.frame(SOURCE=NA, trt_functional=NA, replicates=NA, insect_dens=NA, insect_densSE=NA)
...
lnRRm01=lnRRm01[-c(4,15)]
vlnRRm01=vlnRRm01[-c(4,15)]
...
lnRRm12=lnRRm12[-c(4,14)]
vlnRRm12=vlnRRm12[-c(4,14)]
...
funnel(A01, main="Funnel plot standard/control for insect density")
...
funnel(A02, main="Funnel plot threshold/control for insect density")
...
funnel(A12, main="Funnel plot threshold/standard for insect density")
...
write.table(Re, "C:/Users/kboura01/OneDrive - University of Cyprus/AgStat/meta-analysis/r-codes for metanalysis - final/re-insect_dens.txt", row.names=F)
...
mtext("insect density",3, line=-3, cex=3, oute=TRUE)
